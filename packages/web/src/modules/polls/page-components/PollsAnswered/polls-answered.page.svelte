...
<PageContent rowGap="unset">
  <Surface
    ...
    marginBottom="l4"
  >
    <TitleH1 marginBottom="m">{$LL.polls.pollAnswered()}</TitleH1>

    <TitleH3>{$LL.polls.pollAnsweredMessage()}</TitleH3>
  </Surface>

  <div class="success-container">
    <Paragraph marginBottom="s">
      <TitleH3 fontSize="l2">{$LL.polls.whatDoYouWantToDoNow()}</TitleH3>
    </Paragraph>

    <List listType="disc">
      <ListItem>
        <Link href={'/' + $locale + Path.POLLS}>
          {$LL.polls.goToPolls()}
        </Link>
      </ListItem>
